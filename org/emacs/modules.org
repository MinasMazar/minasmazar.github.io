...
#+begin_src elisp
  ...
  (defun minemacs-jarvis-collect--find-transcript-format (json)
  ...
  (defun minemacs-jarvis-collect--fetch-info (url)
    (with-temp-buffer
      ...
      (let* ((json (json-parse-buffer))
	     ...
	     (data (list
		    ...
		    :script-url (minemacs-jarvis-collect--find-transcript-format json))))
	data)))

  (defun minemacs-jarvis-collect--fetch-transcript (url)
  ...
  (defun minemacs-jarvis-collect (url)
    (let* ((vid (minemacs-jarvis-collect--fetch-info url))
	   (transcript (minemacs-jarvis-collect--fetch-transcript (plist-get vid :script-url)))
	   (vid (plist-put vid :transcript-dom transcript))
	   (id (plist-get vid :id))
	   (buffer-name (format "*<youtube-vid-%s>*" id))
