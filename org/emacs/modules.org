...
*** Feeds
#+begin_src emacs-lisp
  (defvar jarvis-feeds-default-template
    "#+TITLE: %s
  ,#+AUTHOR: %s
  ,#+DATE: <%s>
  ...
  (defun jarvis-feeds-collect-feed (entry feed)
    (let* ((template jarvis-feeds-default-template)
	   (org-content (jarvis-feeds--render template entry)))
      (with-current-buffer (get-buffer-create "*jarvis-feeds-collect*")
	(erase-buffer)
	(insert org-content)
	(org-mode)
	(beginning-of-buffer))))

  (defun jarvis-feeds-collect ()
    (interactive)
    (let* ((count 0))
      (with-elfeed-db-visit (entry feed)
	(setq count (1+ count))
	(setq jarvis-feeds--entry entry jarvis-feeds--feed feed)
	(jarvis-feeds-collect entry feed)
	(when (= count 3)
...
#+begin_src javascript :tangle no
  ...
  window.bifrostSetup = function() {
    ...
    window.bifrostSend(document.body.outerHTML);
  };
...
#+begin_src emacs-lisp
  (use-package web-server
	...
	(with-current-buffer (get-buffer-create "*bifrost*" t)
	  ...
	  (run-hook-with-args 'bifrost-request-hooks request (libxml-parse-html-region)))))
