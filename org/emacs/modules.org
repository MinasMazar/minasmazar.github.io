...
#+begin_src elisp
  ...
  🕓 *Duration:* %s
  ❤️ *Likes:* %s / 👀 *Views:* %s
  " webpage-url title channel-url channel channel-follower-count humanized-duration like-count view-count)))
	   ...
	   (ttml-format (seq-find
			 (lambda (format)
			   (let* ((ext (gethash "ext" format))
				  (res (string= "ttml" ext)))
			     (message (format "<%s> %s" ext res))
			     res))
			 orig-lang))
	   (script-url (and ttml-format (gethash "url" ttml-format))))
      script-url))

  (defun minemacs-jarvis-collector-fetch-info (url)
  ...
  (defun minemacs-jarvis-collector--create-buffer (vid)
    (let* ((transcript (minemacs-jarvis-collector--fetch-transcript (plist-get vid :script-url)))
	   ...
	   (id (plist-get vid :id))
	   (buff-name (minemacs-jarvis-collector--build-filename vid))
	   (coding-system-for-write 'raw-text)
	   (buffer (get-buffer-create buff-name)))
      (with-current-buffer buffer
	(set-visited-file-name (minemacs-jarvis-collector--build-pathname vid))
	(set-buffer-file-coding-system 'raw-text)
	(org-mode)
	(minemacs-jarvis-collector--insert-org-content vid buffer)
	(read-only-mode 1)
	(beginning-of-buffer))
      buffer))

  (defun minemacs-jarvis-collector--get-cache-entry-name (url)
    (file-name-concat minemacs-jarvis-collector-directory ".cache" (md5 url)))
  ...
  (defun minemacs-jarvis-collect (url)
    (interactive "sYoutube video URL: ")
    (let* ((vid (minemacs-jarvis-collector-fetch-info url))
	   (buffer (minemacs-jarvis-collector--create-buffer vid)))
      (pop-to-buffer buffer)
      (with-current-buffer buffer
	(save-buffer)
	buffer)))

  (defun minemacs-jarvis-collect-async (url)
    (interactive "sYoutube video URL: ")
    (async-start
     (lambda () (minemacs-jarvis-collector-fetch-info url))
     (lambda (vid)
       (let ((buffer (minemacs-jarvis-collector--create-buffer vid)))
	 (with-current-buffer buffer
	   (pop-to-buffer buffer)
	   (save-buffer))))))

  ;; (minemacs-jarvis-collect "https://www.youtube.com/watch?v=tOOOC_s5HZ4")
#+end_src
