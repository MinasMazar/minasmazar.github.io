...
* Overrides
:PROPERTIES:
:header-args: :tangle (minemacs-tangle-if-not-disabled "~/.emacs.d/modules/overrides.el") :mkdirp yes
:END:
...
* Custom Helpers
:PROPERTIES:
:header-args: :tangle (minemacs-tangle-if-not-disabled "~/.emacs.d/modules/custom-helpers.el") :mkdirp yes
:END:
...
* Info and Help
:PROPERTIES:
:header-args: :tangle (minemacs-tangle-if-not-disabled "~/.emacs.d/modules/info-help.el") :mkdirp yes
:END:
...
** Collector 🗄️
:PROPERTIES:
:header-args: :tangle (minemacs-tangle-if-not-disabled "~/.emacs.d/modules/jarvis-collector.el") :mkdirp yes :comments link
:END:
...
#+begin_src emacs-lisp
  ...
  (defun minemacs-blank? (str)
    (and str (s-blank? (s-trim str))))
  ...
  (defvar minemacs-jarvis-collector-directory "~/Public/collector"
  ...
  (defun minemacs-jarvis-collector--org-content-description (vid)
    (let ((description (plist-get vid :description)))
      (unless (minemacs-blank? description)
	(format
	 "\n** Description
  ...
  (defun minemacs-jarvis-collector--parse-info (data)
    (let* ((id (gethash "id" data))
	  (vid (list
		 ...
		 :description (s-trim (gethash "description" data))
  ...
  (defun minemacs-jarvis-collector--create-file (vid)
    (let* ((transcript (minemacs-jarvis-collector--fetch-transcript (plist-get vid :script-url)))
	   (vid (plist-put vid :transcript-dom transcript))
	   (id (plist-get vid :id))
  ...
  (defun minemacs-jarvis-collector-fetch-info (url &optional force)
	...
	(with-temp-buffer
	  ...
	  (let* ((json (json-parse-buffer)))
	    ...
	    (minemacs-jarvis-collector--parse-info json))))))
  ...
  (defun minemacs-jarvis-collect (url &optional force-fetch visit)
    (interactive "sYoutube video URL: ")
    (let* ((url (minemacs-jarvis-collector--canonize-url url))
	   (vid (minemacs-jarvis-collector-fetch-info url force-fetch))
	   (pathname (minemacs-jarvis-collector--create-file vid)))
      (and (or visit (called-interactively-p 'any)) (find-file pathname))
      pathname))
#+end_src
...
:PROPERTIES:
:header-args: :tangle (minemacs-tangle-if-not-disabled "~/.emacs.d/modules/exwm.el") :mkdirp yes
:END:
