...
#+begin_src emacs-lisp :tangle (minemacs-tangle-if-not-disabled "jarvis-collector.el") :mkdirp yes :comments link
  ...
  (defun jarvis-collect-video-dashitize-string (str)
    (cl-remove-if
     (lambda (char)
       (> char 127))
  ...
  (defun jarvis-collect-video-collector--build-filename (vid)
    (jarvis-collect-video-dashitize-string (plist-get vid :title)))

  (defun jarvis-collect-video-collector--build-pathname (vid)
...
** Static server                                                    :disable:

#+begin_src emacs-lisp
  (use-package web-server
    :vc (:url "https://github.com/eschulte/emacs-web-server.git")
    :init
    (defcustom minemacs-ws-docroot "~/Public" "Root directory to be served within the File Server")
    :config
    (ws-start
     (lambda (request)
