	   ...
	   (ttml-format (seq-find
			 (lambda (format)
			   (let* ((ext (gethash "ext" format))
			     ...
			     res))
			 orig-lang))
	   (script-url (gethash "url" ttml-format)))
      script-url))
    ...
    (with-temp-buffer
      ...
      (let* ((json (json-parse-buffer))
	     ...
	     (data (list
		    ...
		    :description (s-trim (gethash "description" json))
		    :channel (gethash "channel" json)
		    :channel-url (gethash "channel_url" json)
		    :channel-follower-count (gethash "channel_follower_count" json)
		    :duration (gethash "duration" json)
		    :like-count (gethash "like_count" json)
		    :view-count (gethash "view_count" json)
		    :average-rating (gethash "average_rating" json)
		    :webpage-url (gethash "webpage_url" json)
		    :categories (gethash "categories" json)
		    :tags (gethash "tags" json)
		    :script-url (minemacs-jarvis-collector--find-transcript-format json))))
	data)))
