...
* Emacs as Window Manager                                           :disable:
:PROPERTIES:
...
#+begin_src elisp
  (use-package hydra
    :bind
    ("s-j" . #'hydra-exwm-main-menu/body)
    :config
    (defhydra hydra-exwm-main-menu (:color blue)
      "Main Menu"
      ("<XF86AudioLowerVolume>" #'pulseaudio-control-decrease-sink-volume "🔉dec")
      ("<XF86AudioRaiseVolume>" #'pulseaudio-control-decrease-sink-volume "🔊inc")
      ("<XF86AudioMute" #'pulseaudio-control-toggle-current-sink-mute "🔇mute")
      ("j" #'switch-to-buffer "➡️switch buffer")
      ("w" #'exwm-workspace-switch "↔️switch workspace")
      ("R" #'exwm-reset "🌀Reset")
      ("q" nil)))
#+end_src
...
#+begin_src elisp
    (use-package exwm
      :init
      (async-shell-command "dex -a -d -e exwm" nil "*dex-errors*")
      :config
      (server-mode 1)
      (require 'exwm)
