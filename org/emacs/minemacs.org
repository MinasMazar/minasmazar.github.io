...
More info about =load-path= at https://www.emacswiki.org/emacs/LoadPath. Most of this configuration is inspired by [[https://protesilaos.com/codelog/2024-11-28-basic-emacs-configuration][Protesilaos setup configuration]].
...
#+begin_src elisp
  (use-package emacs
    ...
    (defun minemacs-tangle-if-not-disabled (tangle)
      (if (member-ignore-case "disable" (org-get-tags)) "no" tangle))
