...
* COMMENT Helpers
#+begin_src elisp
  (defun holymacs-with-private-data (&optional body)
    (load "~/.emacs.d/private.el" 'noerror)
    body)

  (defun holymacs-async (cmd)
    (holymacs-inspect cmd)
    (save-window-excursion
      (async-shell-command cmd "*holymacs-async*")))

  (defun holymacs-async (cmd &body)
    (let ((exec-str (format cmd body)))
      (holymacs-inspect cmd)
      (save-window-excursion
        (async-shell-command cmd "*holymacs-async*")))

  (defun holymacs-async-kill ()
    (kill-buffer-ask "*holymacs-async*"))
