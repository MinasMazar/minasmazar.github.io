...
** Set an image as wallpaper using feh
...
#+begin_example conf
  ...
  mode "i3" {
    ...
    bindsym p focus left ; focus up
    bindsym n focus right ; focus down
    bindsym Shift+p workspace prev
    bindsym Shift+n workspace next
    ...
    # Emacs everywhere
    bindsym Shift+m exec --no-startup-id emacsclient --eval "(emacs-everywhere)" && mode "default"
    # Kill current window
...
** [[https://www.instructables.com/How-to-Make-Raspberry-Pi-Webcam-Server-and-Stream-/][How to Make Raspberry Pi Webcam Server and Stream]]
** Turn your RPi into a Spy

Use [[https://motion-project.github.io/][motion]] app.
...
#+begin_src sh
  docker run -d --rm --init -p 3000:3000 --volume=/Users/minasmazar/volumes/busybox:/home/static busybox:1.3.5
#+end_src
...
Go the your "Watch later" page on the browser, open the console and execute

#+begin_src javascript
  var els = document.querySelectorAll('a.ytd-playlist-video-renderer'));
  var elsExp = [];
  els.forEach(function(el) { elsExp.push([el.innerText, el.getAttribute("href")]) });
  console.log(JSON.stringify(elsExp));
#+end_src
...
I've tried to automate this process by downloading the page via CURL, or using Emacs, but it seems that those methods downloads a page that does not have the data we need, maybe because the lack of JS support. A way to avoid that is by using NodeJS as described in [[*Fetch a page via a nodejs app][this section]].
