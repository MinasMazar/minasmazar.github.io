...
- [[https://github.com/puppeteer/puppeteer/issues/4860]["The profile appears to be in use by another Chromium process on another computer"]]
...
/A Makefile tells more than a thousand words/ 😁

#+begin_src makefile
  install-elixir-via-package-manager:
  	sudo apt update
...
More info on ho /install elixir on a Raspberry PI/ in [[https://erlangsolutions.medium.com/installing-elixir-on-a-raspberry-pi-the-easy-way-22f347a77e2c][this article]].
...
Implement the /before/ and /after/ callbacks:
...
My =~/.profile= after having installed guix on an Ubuntu distro.
...
- [[https://wokwi.com/pi-pico][Raspberry Pi Pico Simulator]]
...
Then check in =/usr/share/X11/xorg.conf.d/= for a /calibration/ init file and tweak the =SwapXY= or =InvertX= / =InvertY= options.
If there are issues about the touchscreen which does not move the pointer where you click on the screen, consider using the =evdev= driver as proposed in [[https://forums.adafruit.com/viewtopic.php?f=50&t=111045][this forum]]:
...
Related sources:
- [[https://forums.raspberrypi.com/viewtopic.php?t=336486][Pi0, Single web page Kiosk. Which lightweight browser?]]
...
This script is taken from the [[https://www.otot.tv/fullpageos/][FullPageOS]] and is used to start chromium.
...
It could be userful to add the flag =--ytdl-raw-options=cookies-from-browser=chromium=. Also set =bongo-custom-backend-matchers= to include also =https= protocol. Ensure the Emacs process has the =DISPLAY= env correctly set in order to display the video!
...
To establish an order or requirements across services there are the =Wants / Requires/ After= options. See [[https://serverfault.com/questions/1053369/systemd-how-to-start-service-after-another-service-started][this thread]] for more details.
...
Super simple, basic configuration using [[https://github.com/tmux-plugins/tpm/][TPM]] and [[https://github.com/junegunn/fzf][FZF]]. For a list of ANSI 256 color names you can take a look at [[https://hexdocs.pm/color_palette/ansi_color_codes.html][this link]].
...
To install the packages =prefix+I=.
