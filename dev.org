...
#+begin_src makefile
  ...
  release-start:
  	SECRET_KEY_BASE='rwgfimMVc8/M6qZww/MCFtIWyP5KeXWrjKGbvMIaWp8BvBjm+st31cgsYy0s9wjL' PHX_SERVER=true PHX_HOST=localhost PORT=4020 ~/my-app/bin/my-app start
#+end_src
...
Use Chromium as browser (only on RPI > 1)
...
Midori: works well, but =-e= does not work as expected.
Epiphany: it has interesting options like =automation-mode= but didn't have the time to inspect more.
